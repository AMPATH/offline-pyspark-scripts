...
def get_person_stream(kafka_stream):
...
def start_spark(app_name='my_spark_app', master='local[*]', jar_packages=[],spark_config={}, ssc_config={}, kafka_config={}, callback=None):
    ...
    def createSSC():
        ...
        ssc.checkpoint('checkpoints/patient')
...
#     ssc = StreamingContext.getOrCreate('checkpoints/patient', createSSC)  
    ssc = StreamingContext(sc, ssc_config['batchDuration'])
    kafka_stream = KafkaUtils          .createDirectStream(ssc,topics=kafka_config['topics'],kafkaParams=kafka_config['config'])           .map(lambda msg: json.loads(msg[1]))
    if callback is not None:
        callback(kafka_stream=kafka_stream,sc=sc,spark_conf=spark_conf)
...
def rebuild_microbatch(rdd, spark_conf):
    try:
        ...
        if len(persons) > 0:
            ...
            print("\n --- Micro-Batch --- \n")
            print("Building Person Objects " + time.ctime())
            ...
            query = """(select * from patient_view_2 where person_id in ({0})) foo""".format(",".join([str(i) for i in person_ids]))
            patients = read_from_mysql('amrs', query, spark_conf)                         .alias('patient')
            transformed_patient = transform_patient(patients)
            transformed_patient.show()
            save_to_cassandra(transformed_patient, 'patient')
...
def start_job(kafka_stream, sc, spark_conf):   
        ...
        get_person_stream(kafka_stream).foreachRDD(lambda rdd: rebuild_microbatch(rdd, spark_conf)) 
...
def start():
    ...
    kafka_config = config['kafka-person']
